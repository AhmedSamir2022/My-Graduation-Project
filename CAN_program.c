...
void CAN_voidInit(void)
{
	...
	MGPIO_voidSetPinMode(PORTB,PIN8,GPIO_INPUT_FLOATING);
	MGPIO_voidSetPinMode(PORTB,PIN9,GPIO_AF_OUTPUT_PP_2MHZ);
...
void CAN_voidAddTxMsg( CAN_TxHeaderTypeDef *pTxHeader, u8 Local_u8Data[])
{
	...
	if ( (GET_BIT(CAN->TSR, 26) != 0x0) || /* TME0 */
			(GET_BIT (CAN->TSR,27) != 0x0) || /* TME1 */
			(GET_BIT(CAN->TSR,28) != 0x0) )  /* TME2 */
	{
...
void CAN_voidAddTxMsgInt( CAN_TxHeaderTypeDef *pTxHeader, u32 Copy_u32Data)
{
	uint32 local_u8TransmitMailbox; // which mailbox is empty to set Local_u8Data in it (read CODE bit in TSR register)
	uint32 local_u32TSR; //transmit status register
	local_u32TSR = READ_REG(CAN->TSR);
			...
			(GET_BIT(local_u32TSR,28) != 0x0) )  /* TME2 */
	{
		...
		/* Set the data */
		CAN->TxMailBox[local_u8TransmitMailbox].TDLR = Copy_u32Data;
		/* Request transmission */
		SET_BIT(CAN->TxMailBox[local_u8TransmitMailbox].TIR, 0);
	}
}

void CAN_voidGetRxMsgInt(uint32 local_u8RxFifo, CAN_RxHeaderTypeDef *pRxHeader, u32 *Copy_u32Data)
{
	...
	/* Get the data */
	*Copy_u32Data = CAN->FIFOMailBox[local_u8RxFifo].RDLR;
...
//{ /* init interrput*/
//	//MNVIC_EnableInterrupt(IRQn);
//	//MNVIC_SetPendingFlag(IRQn);
//
...
//{ /* deinit interrput*/
//	//MNVIC_DisableInterrupt(IRQn);
//	//MNVIC_ClearPendingFlag(IRQn);
//
